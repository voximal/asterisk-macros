 ...
 *   https://www.ulex.fr
 *
 * Purpose : Macros to support asterisk 1.8
 */

#ifndef __VERSION_MACROS_AST_1_8__
#define __VERSION_MACROS_AST_1_8__

#warning
#warning Loading macros for ASTERISK_VERSION_NUM == 8
#warning
...
#define SET_CHAN_VSTREAM(chan, value) chan->vstream = value
...
#define GET_AST_REQUEST(pseudo, type, formats, chan, data, reason) 	\
	...
	pseudo = ast_request(type, formats | chan->rawwriteformat, chan, data, reason);
...
#define GET_CHAN_CALLERID_NUMBER(chan)	chan->caller.id.number.str

#define INITIALIZE_SCHED static struct sched_context *sched

#define DELETE_SCHED_ENTRY(sched, id) 	ast_sched_del(sched,id);
...
#define SET_FRAME_SUBCLASS_FORMAT(frame, fmt1)	frame->subclass.codec = fmt1;

#define SET_FRAME_SUBCLASS_FORMAT_AND_VIDEOMARK(frame, fmt1)	frame->subclass.codec = fmt1 | 1

#define GET_CHAN_CALLERID_NAME(chan)	chan->caller.id.name.str

#define SET_CHAN_TECH(chan,value)  chan->tech = value

#define SET_CHAN_TECH_PVT(chan, value) chan->tech_pvt = value
	
#define FRAME_SUBCLASS_FORMAT_CLEAR(frame) frame->subclass.codec = 0

#define GET_FRAME_CONTROL_TYPE(frame) frame->subclass.integer

#define GET_FRAME_SUBCLASS_FORMAT(frame)	frame->subclass.codec

#define GET_FRAME_SUBCLASS_FORMAT_ID(frame)	frame->subclass.codec
...
chan->caller.id.number.str = value

#define SET_CHAN_CALLERID_ANI(chan, value)	\
chan->caller.ani.number.valid = 1;	\
chan->caller.ani.number.str = value

#define COMPARE_VARFORMAT_IDFORMAT(fmt1, fmt2) fmt1 & fmt2

#define SET_CHAN_CALLERID_DNID(chan, value)	\
chan->dialed.number.str = value
...
#define CHECK_FRAME_SUBCLASS_VIDEOMARK(frame) (frame->subclass.codec & 1)
...
#define CALL__RMPT_SHOW_USERS(a,b,c,d) __rtmp_show_users(0, a->fd, a->argc, a->argv)


#endif // __VERSION_MACROS_AST_1_8__
